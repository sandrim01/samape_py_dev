...
# Funções para manipulação de arquivos para o controle de frota
...
def save_image(file, folder, filename=None):
    """
    Salva um arquivo de imagem enviado pelo usuário
    
    Args:
        file: Objeto FileStorage
        folder: Pasta onde a imagem será salva
        filename: Nome do arquivo (opcional, se não for fornecido será gerado um UUID)
        
    Returns:
        Caminho relativo para o arquivo salvo
    """
    from flask import current_app
    from werkzeug.utils import secure_filename
    import uuid
    ...
    # Retornar caminho relativo a partir da pasta static
    static_folder = current_app.static_folder
    if filepath.startswith(static_folder):
        return filepath[len(static_folder):].lstrip('/')
