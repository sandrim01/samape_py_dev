...
def register_routes(app):
    ...
    @login_required
    def get_service_order_data(id):
        """Retorna os dados de uma OS em formato JSON para o modal"""
        try:
            # Consulta direta sem joins para evitar problemas com o PostgreSQL
            conn = db.engine.raw_connection()
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, description, status, created_at, closed_at, client_id, responsible_id, equipment_id FROM service_order WHERE id = %s", 
                (id,)
            )
            order = cursor.fetchone()
            ...
            # Dados básicos da OS
            order_id = order[0]
            description = order[1] or ""
            status = order[2]
            created_at = order[3].strftime('%d/%m/%Y %H:%M') if order[3] else 'Não definido'
            closed_at = order[4].strftime('%d/%m/%Y %H:%M') if order[4] else 'Não definido'
            client_id = order[5]
            responsible_id = order[6]
            ...
            # Recuperar nome do cliente e responsável
            client_name = "Cliente não especificado"
            responsible_name = "Responsável não especificado"
            ...
            try:
                ...
                if responsible_result:
                    responsible_name = responsible_result[0]
            except Exception as e:
                app.logger.error(f"Erro ao buscar responsável: {str(e)}")
            ...
            try:
                cursor.execute("""
                    SELECT 
                        ...
                        SUM(CASE WHEN type = 'receita' THEN original_amount ELSE 0 END) as original,
                        SUM(CASE WHEN type = 'receita' THEN discount_amount ELSE 0 END) as discount
                    FROM financial_entry
                    WHERE service_order_id = %s
                """, (id,))
                financial = cursor.fetchone()
                if financial:
                    total_value = financial[0] or 0
                    original_amount = financial[1] or 0
                    discount_amount = financial[2] or 0
            except Exception as e:
                app.logger.error(f"Erro ao buscar dados financeiros: {str(e)}")
            ...
            data = {
                ...
                'view_url': url_for('view_service_order_basic', id=id),
                'edit_url': url_for('edit_service_order', id=id)
            }
            ...
            app.logger.error(f"Erro ao obter dados da OS {id}: {str(e)}")
